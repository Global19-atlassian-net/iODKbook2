...
Problem własny w układach równań różniczkowych - przykłady
----------------------------------------------------------

Omówimy teraz różne sytuacje, odpowiadające 
...
:math:`\ \boldsymbol{v}_1,\,\boldsymbol{v}_2,\,\ldots,\,\boldsymbol{v}_n\,,` 
:math:`\\` a także odpowiadające im rozwiązania szczególne

.. math::
   :label: spec_sols

   \boldsymbol{x}^1(t)=e^{\,\lambda_1\,t}\,\boldsymbol{v}_1\,,\quad
   \boldsymbol{x}^2(t)=e^{\,\lambda_2\,t}\,\boldsymbol{v}_2\,,\quad
   \ldots\,,\quad
...
.. math::
   ...
   \boldsymbol{x}(t)\ =
   \ c_1\ \boldsymbol{x}^1(t)\,+\,c_2\ \boldsymbol{x}^2(t)\,+\,\ldots\,+\,
...
.. math::
   ...
   \boldsymbol{v}_2\,=\,\beta\ 
   \left[\begin{array}{c} 1 \\ 4 \end{array}\right]
   \,, & \beta\in R\!\smallsetminus\!\{0\}\,.
   \end{array}

Kładąc :math:`\,\beta=1\ ` otrzymujemy dwa liniowo niezależne 
...
.. math::
   ...
   \begin{array}{c}
   \boldsymbol{x}(t)\,=
   \,c_1\ \boldsymbol{x}^1(t)\,+\,c_2\ \boldsymbol{x}^2(t)\ : \\ \\
...
Przede wszystkim zauważmy, że skoro macierz :math:`\,\boldsymbol{A}\ ` 
jest rzeczywista, to zespolone nierzeczywiste pierwiastki równania 
charakterystycznego występują parami:
...
to :math:`\ \boldsymbol{v}^*\ ` jest wektorem własnym dla wartości 
:math:`\ \lambda^*:`
...
.. math::
   ...
   \ \left[\,e^{\,\lambda\,t}\;\boldsymbol{v}\,\right]^*\,.

Rozwiązanie 
:math:`\ \,\boldsymbol{x}(t)\,=\,e^{\,\lambda\,t}\,\boldsymbol{v}\,,\ `
...
i z przyrównania części rzeczywistych oraz części urojonych 
skrajnych wyrażeń wynika, że
...
**Ćwiczenie.** :math:`\,`
W uzupełnieniu dyskusji Przypadków 
:math:`\,` 1. :math:`\,` i :math:`\,` 2. :math:`\,` udowodnij, że:

1. :math:`\,` Jeżeli wektory 
   :math:`\ \boldsymbol{v}_1,\,\boldsymbol{v}_2,\,\ldots,\,
   \boldsymbol{v}_n\in C^n\ ` są liniowo niezależne, to dla 
   :math:`\ \alpha_i\in C\!\smallsetminus\!\{0\}\,,\ `
   :math:`i=1,2,\ldots,n\,,\ \,` liniowo niezależne są również wektory 
   :math:`\ \ \alpha_1\,\boldsymbol{v}_1,\ \ \alpha_2\,\boldsymbol{v}_2,
   \ \ldots,\ \alpha_n\,\boldsymbol{v}_n` :math:`\\` (w wyrażeniach 
   :eq:`spec_sols` dla rozwiązań szczególnych
   :math:`\ \alpha_i=\exp{(\lambda_i\,t)}\,,\ i=1,2,\ldots,n`).

2. :math:`\,` Jeżeli wektor :math:`\ \boldsymbol{x}\in C^n\ ` ma postać
   :math:`\ \boldsymbol{x}=\boldsymbol{x}_1+i\ \boldsymbol{x}_2\,,\ \,
   \text{gdzie}\ \,\boldsymbol{x}_1,\boldsymbol{x}_2\in R^n\,,\ ` to liniowa 
   niezależność wektorów :math:`\ \boldsymbol{x},\,\boldsymbol{x}^*\ ` jest 
   równoważna liniowej niezależności wektorów 
   :math:`\ \boldsymbol{x}_1,\boldsymbol{x}_2\,.`
...
.. math::
   ...
   \end{array}\right|\ \,=\ \, \lambda^2-6\,\lambda+10\ \,=\ \,0
...
ma postać 
:math:`\ \boldsymbol{x}^1(t)=\boldsymbol{x}_1(t)+i\ \boldsymbol{x}_2(t)\,,\ `
gdzie :math:`\ \boldsymbol{x}_1(t)\,,\ \boldsymbol{x}_2(t)\ ` 
są funkcjami o wartościach w :math:`\ R^2\,.` :math:`\\`

Ponieważ część rzeczywista i część urojona rozwiązania zespolonego jest, 
każda z osobna, rozwiązaniem układu, to ogólne rozwiązanie dane jest przez 
dowolną ich kombinację liniową:

.. math::
   
   \begin{array}{c}
   \boldsymbol{x}(t)\ =
   \ c_1\ \boldsymbol{x}_1(t)\ +\ c_2\ \boldsymbol{x}_2(t)\ :
   \\ \\
   \left[\begin{array}{c} x_1(t) \\ x_2(t) \end{array}\right]\ \ =\ \ 
   e^{\,3\,t}\ \left(\ 
...
Niektóre wartości własne macierzy :math:`\,\boldsymbol{A}\ ` są pierwiastkami 
wielokrotnymi jej wielomianu charakterystycznego, przy czym krotności 
geometryczne takich wartości równają się odpowiednim krotnościom algebraicznym.
Oznacza to, że dla każdego :math:`\,k`-krotnego pierwiastka wielomianu 
charakterystycznego istnieje :math:`\,k\ ` liniowo niezależnych wektorów 
własnych macierzy :math:`\,\boldsymbol{A}\,.`

.. Sytuacja ta nie wymaga wprowadzania nowych elementów 
   do postępowania opisanego w przypadkach 1. i 2.
...
Dla wartości własnej :math:`\,\lambda_{1,2}\ ` układ równań 
:eq:`hom_set` redukuje się do
...
.. math::
   ...
   \boldsymbol{v}_{1,2}\ =\ 
   \left[\begin{array}{c} 
      \beta_1 \\ \beta_2 \\ \beta_1-2\,\beta_2 \end{array}\right]
   \ =\ \beta_1\ 
   \left[\begin{array}{r} 1 \\ 0 \\ 1 \end{array}\right]
   \ +\ \beta_2\ 
   \left[\begin{array}{r} 0 \\ 1 \\ -2 \end{array}\right]\,,\qquad
   \begin{array}{c} 
      \beta_1,\,\beta_2\in R\,, \\ \beta_1^2+\beta_2^2>0 
   \end{array}

tworzą (łącznie z wektorem zerowym) podprzestrzeń 2-wymiarową.

Wartości :math:`\,\lambda_{1,2}=1\ ` odpowiadają więc dwa liniowo niezależne 
rozwiązania szczególne:

.. math::
   :label: sol_12
   
   \boldsymbol{x}^1(t)\ \,=\ \,e^{\,t}\ 
   \left[\begin{array}{r} 1 \\ 0 \\ 1 \end{array}\right]
   \qquad\text{oraz}\qquad
   \boldsymbol{x}^2(t)\ \,=\ \,e^{\,t}\ 
   \left[\begin{array}{r} 0 \\ 1 \\ -2 \end{array}\right]\,.

Wektory własne macierzy :math:`\,\boldsymbol{A}\ ` dla wartości 
:math:`\,\lambda_3=-2\ ` mają postać
...
.. math::
   ...
   \boldsymbol{x}^3(t)\ \,=\ \,e^{\,-2\,t}\ 
   \left[\begin{array}{r} 3 \\ 3 \\ -4 \end{array}\right]\,.
...
Dla niektórych wartości własnych macierzy :math:`\,\boldsymbol{A}\ ` 
krotność geometryczna jest różna (mniejsza) od krotności algebraicznej.

.. Chociaż nie istnieje wtedy baza przestrzeni :math:`\,R^n\ ` złożona 
   z wektorów własnych macierzy :math:`\,\boldsymbol{A},\ ` to można 
   skonstruować *bazę Jordana* tej przestrzeni. Wykorzystując wektory tej bazy 
   można utworzyć zbiór :math:`\,n\ ` liniowo niezależnych rzeczywistych 
   rozwiązań układu :eq:`set_diff`.
...
Wektory 
:math:`\,\boldsymbol{w}_1,\boldsymbol{w}_2\in R^3
\!\smallsetminus\!\{\boldsymbol{0}\}\ ` 
...
.. math::
   ...
   \begin{cases}\ \begin{array}{l}
      (\boldsymbol{A}-\lambda_{1,2}\ \boldsymbol{I}_3)\ \boldsymbol{w}_1\ =
      \ \boldsymbol{0} \\
      (\boldsymbol{A}-\lambda_{1,2}\ \boldsymbol{I}_3)\ \boldsymbol{w}_2\ =
      \ \boldsymbol{w}_1
   \end{array}\end{cases}
   \quad\text{czyli}\qquad\ 
   \begin{cases}\ \begin{array}{l}
      \boldsymbol{A}\,\boldsymbol{w}_1\ =\ \lambda_{1,2}\ \boldsymbol{w}_1 \\
      \boldsymbol{A}\,\boldsymbol{w}_2\ =
      \ \boldsymbol{w}_1+\lambda_{1,2}\ \boldsymbol{w}_2
   \end{array}\end{cases}

Pokażemy, że :math:`\ \,\boldsymbol{w}_1,\,\boldsymbol{w}_2\ \,` 
są liniowo niezależne. Rzeczywiście, niech

.. math::
   
   \alpha_1\ \boldsymbol{w}_1\ +\ \alpha_2\ \boldsymbol{w}_2\ \,=
   \ \,\boldsymbol{0}\,,\qquad\alpha_1,\,\alpha_2\in R\,.
...
.. math::
   ...
   \begin{eqnarray*}
   \alpha_1\ (\boldsymbol{A}-\lambda_{1,2}\ \boldsymbol{I}_3)
   \ \boldsymbol{w}_1\ +\ 
   \alpha_2\ (\boldsymbol{A}-\lambda_{1,2}\ \boldsymbol{I}_3)\ \boldsymbol{w}_2 
   & = & \boldsymbol{0} \\ 
   \alpha_2\ \boldsymbol{w}_1 & = & \boldsymbol{0}\,,\quad\text{skąd}\quad
   \alpha_2=0\,, \\
...
jest rozwiązaniem rozważanego układu równań różniczkowych. 
Istotnie, wobec :eq:`w1_w2` mamy
...
.. math::
   ...
   \lambda_{1,2}\ \exp{(\lambda_{1,2}\;t)}\,\cdot\,(t\,\boldsymbol{w}_1\,+
   \,\boldsymbol{w}_2)\ +
   ...
   & = & \exp{(\lambda_{1,2}\;t)}\,\cdot\, 
   (\ t\,\cdot\,\boldsymbol{A}\,\boldsymbol{w}_1\,+
   \,\boldsymbol{A}\,\boldsymbol{w}_2\ )\ = \\
...
Wyznaczymy teraz wektory 
:math:`\,\boldsymbol{w}_1\ \ \text{i}\ \ \boldsymbol{w}_2\,.\ `
...
:math:`\ \ (\boldsymbol{A}-\lambda_{1,2}\,\boldsymbol{I}_3)\,\boldsymbol{w}_2=
\boldsymbol{w}_1\,,\ \ ` czyli
...
Rozwiązaniem jest: 
:math:`\ \ \gamma_1=1,\ \ \gamma_2=\gamma,\ \ \gamma_3=-1-\gamma,
\quad\gamma\in R.\ \,` Dla :math:`\ \gamma=0\ ` otrzymujemy
...
Rozwiązanie :eq:`x2_compact` układu równań różniczkowych 
przyjmuje teraz konkretną postać:
...
Stąd:
:math:`\quad\beta_1=3\,\beta\,,\ \ \beta_2=4\,\beta\,,\ \ \beta_3=2\,\beta\,,
\ \ \beta\in R\,,\quad` więc 
:math:`\quad\boldsymbol{v}_3\ =
\ \beta\ \left[\begin{array}{c} 3 \\ 4 \\ 2 \end{array}\right]\,,
\ \ \beta\in R\!\smallsetminus\!\{0\}\,,`

a rozwiązanie układu równań różniczkowych 
dla tej wartości własnej dane jest przez
...
Wektor :math:`\,\boldsymbol{v}_3\ ` (np. przy :math:`\,\beta=1`) 
może być przyjęty jako trzeci wektor :math:`\,\boldsymbol{w}_3\ ` 
bazy Jordana w :math:`\,R^3\,,\ ` odpowiadającej macierzy 
:math:`\,\boldsymbol{A}:`
